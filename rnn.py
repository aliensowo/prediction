...
def get_data():
    data = requests.get('https://www.alphavantage.co/query?'
                        ...
                        'symbol={}&'
                        'outputsize=full&'
                        'apikey={}'.format('GOOG', 'AWRI5P1983OG0FXN')).json()
    ...
    # stock = stock.reindex(columns=['Date', 'Adj. Close', 'Adj. Open', 'High', 'Low'])
    # stock.drop(stock.columns[[0]], axis='columns')
    return stock
...
def split_dataset(data):
    ...
    train = data[1:-1140]  # AAPL train = data[-1780:-2] test = data[1:-1780]
    test = data[-1140:-6]
...
# data.to_csv('out.csv')
# load the new file
dataset = read_csv('out.csv', header=0, infer_datetime_format=True,
                   parse_dates=['Date'], index_col=['Date'])

# split into train and test
train, test = split_dataset(dataset.values)
# define the number of subsequences and the length of subsequences
n_steps, n_length = 10, 7
